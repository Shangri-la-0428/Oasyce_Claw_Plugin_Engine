...
class DeviceAttestation(BaseModel):
    ...
    device_id: str = Field(..., description="Unique device identifier (e.g., iOS IDFV or Android ID)")
    ...
    certificate_chain: List[str] = Field(
        ...
        description="X.509 certificate chain from device TEE, base64 encoded"
    )
    attestation_token: Optional[str] = Field(
        None,
        description="Platform attestation token (Apple DeviceCheck / Android SafetyNet)"
    )
...
class SensorVector(BaseModel):
    ...
    """
    gyro_x: List[float] = Field(..., min_length=10, description="Gyroscope X-axis samples during capture")
    gyro_y: List[float] = Field(..., min_length=10, description="Gyroscope Y-axis samples during capture")
    gyro_z: List[float] = Field(..., min_length=10, description="Gyroscope Z-axis samples during capture")
    accel_magnitude: Optional[List[float]] = Field(None, description="Accelerometer magnitude samples")
...
class OTPPayload(BaseModel):
    ...
    signature: str = Field(..., description="ECDSA signature over (content_hash + timestamp + geo_hash)")
...
class VerificationReport(BaseModel):
    ...
    can_be_public: bool = Field(default=False, description="Whether this asset can enter the public pool")
