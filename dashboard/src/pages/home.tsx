...
export default function Home({ go }: { go: (p: Page, sub?: string) => void }) {
  ...
  const modeSwitch = (label: string) => (
    <div class="home-mode-switch" role="tablist" aria-label={label}>
      <button role="tab" aria-selected={mode === 'data'}
        ...
        onClick={() => setMode('data')}>{_['register-data']}</button>
      ...
      <button role="tab" aria-selected={mode === 'capability'}
        ...
        onClick={() => setMode('capability')}>{_['publish-cap']}</button>
    </div>
  );
  ...
  return (
    <div class="page home-page">
      ...
      {/* Gate / Register / Success */}
      {!walletExists && (
        <>
          <h2 class="home-section-title">{copy.walletGateTitle}</h2>
      ...
      {walletExists && !hasStarterFunds && (
        <>
          <h2 class="home-section-title">{copy.fundsGateTitle}</h2>
          ...
          {done ? (
            <>
              <div class="home-success-banner">
                ...
                <h2 class="home-section-title">{done.capability ? copy.successTitleCap : copy.successTitleData}</h2>
              </div>
