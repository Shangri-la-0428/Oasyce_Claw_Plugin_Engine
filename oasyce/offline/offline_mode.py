...
class OfflineDetector:
    """Minimal stub replacing the deleted consensus.network.offline_detector.

    Checks basic internet reachability and returns a simple status string.
    """

    _PROBE_URL = "https://api.oasyce.com/health"
    _TIMEOUT = 3  # seconds

    def get_status(self) -> str:
        """Return 'online', 'degraded', or 'offline'."""
        import urllib.request
        import urllib.error

        try:
            urllib.request.urlopen(self._PROBE_URL, timeout=self._TIMEOUT)
            return "online"
