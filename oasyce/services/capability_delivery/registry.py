...
class EndpointRegistry:
    ...
    def register(
        ...
        if not self._allow_private:
            from oasyce.services.capability_delivery.gateway import _validate_endpoint_url
    ...
    def list_active(
        self, provider_id: Optional[str] = None, tag: Optional[str] = None, limit: int = 50
    ...
    def update_stats(
        self, capability_id: str, latency_ms: float, success: bool, earned: int = 0
    ) -> None:
        ...
        with self._lock:
            ...
            self._conn.execute(
                ...
                WHERE capability_id = ?
            """,
                (new_n, new_latency, new_rate, old_earned + earned, capability_id),
            )
            self._conn.commit()
    ...
    def delist(self, capability_id: str) -> bool:
        ...
        with self._lock:
            cur = self._conn.execute(
                "UPDATE capability_endpoints SET status = 'delisted' " "WHERE capability_id = ?",
                (capability_id,),
            )
            self._conn.commit()
            return cur.rowcount > 0
