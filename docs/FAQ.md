...
| 1 | PoW 注册 | 钱包已创建 | 链上身份 + 20 OAS 空投（债务） |
| 2 | 领取测试代币 | 已注册 | +10,000 OAS（测试网，24h 冷却） |
| 3 | 扫描本地数据 | DataVault 已安装 | 发现可注册的安全文件 |
| 4 | 注册数据资产 | 有钱包 + 有文件 | 链上资产，bonding curve 初始化 |
| 5 | 注册 AI 能力 | 有钱包 + 有 API 端点 | 能力进入全局发现池 |
| 6 | 搜索/发现资产或能力 | 无（公开查询） | 排序后的结果列表 |
| 7 | 买入数据份额 | 余额足够 | 获得份额 → 解锁对应访问级别 |
| 8 | 调用 AI 能力 | 余额足够 + 信誉达标 | 获得计算结果 |
| 9 | 卖出数据份额 | 持有份额 | 反向曲线变现（95% reserve 上限） |
| 10 | 提交争议 | 有交互记录 | 5 人陪审团投票裁决 |
| 11 | 偿还注册债务 | 有余额 | 20 OAS 被销毁，释放可用余额 |
| 12 | 质押成为验证者 | ≥10,000 OAS | 参与共识出块 |
| 13 | 配置 Agent 自动化 | 已注册 | 定时扫描、自动交易、规则引擎 |
