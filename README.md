...
pip install oasyce
oasyce doctor            # 健康检查
oasyce start             # Dashboard: localhost:8420
```
...
如需 API 服务（程序化调用），另开终端运行 `oasyce serve`。
...
```
oasyce task post "翻译这份文档" --budget 50 --deadline 3600
oasyce task list                                  # 查看所有任务
oasyce task bid TASK_ID --price 30 --seconds 1800 # 竞标
oasyce task select TASK_ID --agent AGENT_ID       # 选择中标者
oasyce task complete TASK_ID                      # 标记完成
oasyce task cancel TASK_ID                        # 取消任务
```

### AI 反馈

```
oasyce feedback "购买流程有 bug" --type bug --agent my-agent
oasyce feedback "建议增加批量导入" --type suggestion --json
```
...
oasyce start --no-browser # 启动 Dashboard（不自动开浏览器）
...
运行 `oasyce start` 后，浏览器自动打开 `http://localhost:8420`：
...
| **oasyce** (本仓库) | v2.3.0 | 1083 | 功能完整，架构约束强制执行 |
