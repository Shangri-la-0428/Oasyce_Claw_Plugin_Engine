...
class TestStaking:
    def test_stake_success(self, engine):
        v = engine.stake("node-0", "pk_0", 2000.0)
        assert v.stake == 2000.0
        assert v.status == ValidatorStatus.ACTIVE
        assert engine.total_staked == 2000.0
    ...
    def test_stake_adds_to_existing(self, engine):
        engine.stake("node-0", "pk_0", 1000.0)
        engine.stake("node-0", "pk_0", 500.0)
        assert engine.validators["node-0"].stake == 1500.0
        assert engine.total_staked == 1500.0

    def test_multiple_validators(self, engine):
        engine.stake("node-0", "pk_0", 1000.0)
        engine.stake("node-1", "pk_1", 3000.0)
        assert len(engine.active_validators()) == 2
        assert engine.total_active_stake() == 4000.0


class TestUnstaking:
    def test_unbonding_period(self, engine):
        engine.stake("node-0", "pk_0", 2000.0)
...
class TestBlockRewards:
    def test_initial_reward(self, engine):
        assert engine.block_reward_amount(0) == 50.0

    def test_halving(self, engine):
        # After 525600 blocks (~1 year)
        assert engine.block_reward_amount(525_600) == 25.0
        assert engine.block_reward_amount(525_600 * 2) == 12.5
        assert engine.block_reward_amount(525_600 * 3) == 6.25

    def test_distribute_reward(self, engine):
        engine.stake("node-0", "pk_0", 2000.0)
        event = engine.distribute_block_reward("node-0", block_number=0, tx_fees=100.0)
        # Block reward 50 + validator share 20% of 100 = 20
        assert event.block_reward == 50.0
        assert event.fee_reward == 20.0
        assert event.total == 70.0
        assert engine.validators["node-0"].rewards_earned == 70.0
    ...
    def test_fee_distribution(self, engine):
        ...
        assert split["creator"] == 70.0
        assert split["validators"] == 20.0
        assert split["burn"] == 10.0
...
class TestSlashing:
    ...
    def test_slash_double_block(self, engine):
        engine.stake("node-0", "pk_0", 4000.0)
        event = engine.slash("node-0", SlashReason.DOUBLE_BLOCK)
        assert event.amount_slashed == 2000.0  # 50%
        assert engine.validators["node-0"].stake == 2000.0

    def test_slash_offline(self, engine):
        engine.stake("node-0", "pk_0", 2000.0)
        event = engine.slash("node-0", SlashReason.PROLONGED_OFFLINE)
        assert event.amount_slashed == 100.0  # 5%
        assert engine.validators["node-0"].stake == 1900.0

    def test_slash_below_minimum_forces_exit(self, engine):
        engine.stake("node-0", "pk_0", 1000.0)
        engine.slash("node-0", SlashReason.DOUBLE_BLOCK)  # 50% = 500, below 1000 min
    ...
    def test_detect_double_block(self, engine):
        ...
        assert engine.validators["node-0"].slashed_amount == 2000.0
...
class TestValidatorSelection:
    ...
    def test_select_favors_higher_stake(self, engine):
        engine.stake("node-0", "pk_0", 1000.0)
        engine.stake("node-1", "pk_1", 9000.0)
    ...
    def test_select_balances_over_time(self, engine):
        engine.stake("node-0", "pk_0", 2000.0)
        engine.stake("node-1", "pk_1", 2000.0)
    ...
    def test_slashed_not_selected(self, engine):
        engine.stake("node-0", "pk_0", 5000.0)
        engine.stake("node-1", "pk_1", 1000.0)
        engine.slash("node-0", SlashReason.MALICIOUS_BLOCK)
        selected = engine.select_block_producer()
        assert selected == "node-1"
...
class TestNetworkStats:
    ...
    def test_stats_after_activity(self, engine):
        engine.stake("node-0", "pk_0", 3000.0)
        engine.stake("node-1", "pk_1", 2000.0)
        ...
        assert stats["active_validators"] == 2  # node-1 still active (stake > min)
...
class TestIncentiveAlignment:
    ...
    def test_honest_validator_profits(self, engine):
        ...
        engine.stake("honest", "pk_h", 5000.0)
        for i in range(10):
    ...
    def test_malicious_validator_loses_everything(self, engine):
        ...
        engine.stake("evil", "pk_e", 10000.0)
        ...
        assert v.stake == 0.0
        assert v.slashed_amount == 10000.0
        # Net negative: lost 10000 stake, earned only ~70 rewards
    ...
    def test_attack_cost_exceeds_benefit(self, engine):
        ...
        engine.stake("attacker", "pk_a", 5000.0)
        ...
        # Loss always > gain (5000 stake lost vs ~210 earned)
    ...
    def test_everyone_is_a_stakeholder(self, engine):
        ...
        engine.stake("node-0", "pk_0", 2000.0)
        engine.stake("node-1", "pk_1", 3000.0)
        engine.stake("node-2", "pk_2", 5000.0)

        # Total staked = total at risk = total alignment
        assert engine.total_active_stake() == 10000.0
